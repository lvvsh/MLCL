...
def DLinfo_nce_loss(feats):
    gathered_feats = feats
    cos_sim = F.cosine_similarity(
        gathered_feats[:, None, :], gathered_feats[None, :, :], dim=-1
    )
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders,noloader):

        self.dataloaders = dataloaders
        self.noloader = noloader
        self.infornce = InfoNCE()
    ...
    def _forward_pass(self, args,batch,nobatch):
        self.batch = batch
        ...
        img_in3 = nobatch['A'].to(self.device)
        img_in4 = nobatch['B'].to(self.device)
        img_in34 = torch.abs(img_in3 - img_in4)
        ...
        if args.loss_SD== True :
            ...
            #对比方法
            self.G_pred ,self.x1,self.x2,self.x3,self.x4,self.catx1,self.catx2,self.fdl,self.tdl= self.net_G(img_in1, img_in2,img_in3, img_in4,img_in12)
            #IFNet专用
    ...
    def _backward_G(self,args):
        ...
        sslloss1 = self.infornce(self.x1,self.x3)
        sslloss1_ = self.infornce(self.x2,self.x4)
        sslloss2 = self.infornce(self.fdl,self.tdl)
        self.G_loss=0
        if args.loss_SD == True:
            # self.G_loss = self._pxl_loss1(self.G_pred1, gt,epoch_id,epoch_max) + 0.5*self._pxl_loss2(self.G_pred2, gt) + 0.5 * self._pxl_loss2(self.G_pred3, gt)
            #IFNet
            ...
            if args.deep_supervision == True:
                for pred in self.G_pred:
                    self.G_loss += self._pxl_loss(pred, gt)
                self.G_loss /=len(self.G_pred)
                self.G_loss = self.G_loss+sslloss1+sslloss1_+sslloss2
            else:
                self.G_loss = self._pxl_loss(self.G_pred, gt)
                self.G_loss = self.G_loss+sslloss1+sslloss1_+sslloss2
    ...
    def train_models(self,args):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            # Iterate over data.
            train_loader_len = len(self.dataloaders['train'])
            no_loader_len = len(self.noloader)
            total = max(train_loader_len, no_loader_len)
            self.logger.write('lr: %0.7f\n' % self.optimizer_G.param_groups[0]['lr'])
            for train_batch, no_batch in tqdm(zip_longest(self.dataloaders['train'], self.noloader, fillvalue=None), total=total):
                self._forward_pass(args,train_batch,no_batch)
                # update G
            ...
            for self.batch_id, batch in enumerate(self.dataloaders['val'], 0):
                with torch.no_grad():
                    self._forward_pass(args,batch,batch)
                self._collect_running_batch_states(args)
            self._collect_epoch_states()
