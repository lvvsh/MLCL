...
def get_sobel_path(root_dir,split,img_name):
    return os.path.join(root_dir,split, IMG_SOBEL_FOLDER_NAME, img_name)
...
def get_label_path(root_dir,split, img_name):
    return os.path.join(root_dir, split,ANNOT_FOLDER_NAME, img_name.replace('.png', label_suffix))
...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        A_path = get_img_path(self.root_dir,self.split, self.img_name_list[index % self.A_size])#得到时相1影像的文件路径
        B_path = get_img_post_path(self.root_dir, self.split,self.img_name_list[index % self.A_size])

        img = np.asarray(Image.open(A_path).convert('RGB'))#时相1
        img_B = np.asarray(Image.open(B_path).convert('RGB'))#时相2
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        #sobel差分特征图
        # sobel_path = get_sobel_path(self.root_dir,self.split, self.img_name_list[index % self.A_size])
        # sobel = np.asarray(Image.open(sobel_path).convert('RGB'))
        ...
        if self.label_transform == 'norm':
            label = label // 255
        if self.label_transform == 'no':
            label = label // 255
            [img, img_B], [label] = self.augm.transformno([img, img_B], [label], to_tensor=self.to_tensor)
            return {'name': name, 'A': img, 'B': img_B, 'L': label}

        #
        # [img, img_B,sobel], [label] = self.augm.transform([img, img_B,sobel], [label], to_tensor=self.to_tensor)
        [img, img_B], [label] = self.augm.transform([img, img_B], [label], to_tensor=self.to_tensor)
        # print(label.max())
        # return {'name': name, 'A': img, 'B': img_B,'L':label, 'S': sobel}
        return {'name': name, 'A': img, 'B': img_B, 'L': label}
